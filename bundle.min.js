...
c.value="/* JSON */\n"+e.map(function(n){return JSON.stringify(n)}).join("\n\n").trim(),a.value="(); Generated from right\n"+t.map(function(n){return s.explain(n)}).join("").trim()}function r(){try{i()}catch(n){throw c.value=n.toString(),console.error(n),n}}var s=n("./parser.js"),o=(console.log,function(n){return document.getElementById(n)}),u=o("input"),c=o("json"),a=o("lisp")
...
c(o(!1,"1f",!0,"2f","3f"))
var h=("\n__\n= x 2\n= x\n  + 3 2 y\n\n= x (+ 3 2 y\n* x (+ 1.5 6 (* 2 3\n\n+ (* 3 3) (* 4 4\n  * 5 5\n  * 6 6\n\n+\n  (3) 4 5\n  3 4 5\n  (3 4 5\n\n* (+ 1 2)(+ 3 4)\n___\n+ 'holy' 'fuc ing' spaces\nprint x\nwhile\n__> x 0.2\n__= x\n____- x 178\n__\niif\n__> x 2\n__- x 1\n__+ x\n____* 5 x".replace(/_/g," "),function(n){throw new Error(n)}),p=r(function(n){n.init=function(n,t){if("symbol"===n)this.symbol=t
...
this.emit("push")}},n.end_indent=function(){for(;0!==this.stack.length;)this.pop(),this.emit("pop")}}),k=function(){function n(){u(M.smap)}function t(n){c("begin parse"),f(),T=[]
for(var t=0;t<n.length;t++)M.eat(n[t])
return M.eat("\0"),c("end parse"),T}var e=new _,i=new w,s=new m,a=new g,f=function(){s.reset(),e.reset(),i.reset(),M.reset(),a.reset(),l.reset()},l=new b
s.on("symbol",function(n){e.append(new p("symbol",n))}),a.on("string",function(n){e.append(new p("string",n))})
var d=function(){i.end_indent()}
i.on("push",function(){e.push()}),i.on("pop",function(){e.pop()}),l.on("inc",function(){e.push()}).on("dec",function(){e.pop()})
var v=function(n,t){return t.indexOf(n)>=0},y=function(n){return function(t){return t===n}},k=function(n){return"a"<=n&&n<="z"||"A"<=n&&n<="Z"||"0"<=n&&n<="9"||v(n,":.<>?=_-+*/!%^&@$\\")},x=y(" "),L=y("\n"),q=y("\0"),E=y("'"),j=function(n){return"quoting"===M.state&&!(L(n)||q(n)||E(n))},O=y("("),A=y(")"),P=function(n){return o(j(n),"string",O(n),"lbr",A(n),"rbr",k(n),"symbol",x(n),"space",L(n),"newline",q(n),"eos",E(n),"quote","invalid-category")},N=r(function(n){n.init=function(){this.reset(),this.smap={}},n.reset=function(){this.state="linestart"},n.eat=function(n){var t=P(n),e=this.smap[this.state]
...
return i}}),M=new N
M.reg("linestart")("symbol","symboling",function(n){i.start(),i.end(),s.start(),s.in(n)})("space","indenting",function(n){i.start(),i.in()}),M.reg("indenting")("symbol","symboling",function(n){i.end(),s.start(),s.in(n)})("space","indenting",function(n){i.in()})("quote","quoting",function(n){i.end(),a.start()}),M.reg("symboling")("symbol","symboling",function(n){s.in(n)})("space","spacing",function(n){s.end()})("newline","linestart",function(n){s.end(),l.closebraces()}),M.reg("spacing")("symbol","symboling",function(n){s.start(),s.in(n)})("space","spacing",function(n){})("quote","quoting",function(n){a.start()}),M.reg("quoting")("string","quoting",function(n){a.in(n)})("quote","quoted",function(n){a.end()}),M.reg("quoted")("space","spacing",function(n){}),M.reg("symboling")("lbr","spacing",function(n){s.end(),l.inc()}),M.reg("linestart")("lbr","spacing",function(n){i.start(),i.end(),l.inc()}),M.reg("indenting")("lbr","spacing",function(n){i.end(),l.inc()}),M.reg(["spacing","quoted"])("lbr","spacing",function(n){l.inc()}),M.reg("symboling")("rbr","spacing",function(n){s.end(),l.dec()}),M.reg(["spacing","quoted"])("rbr","spacing",function(n){l.dec()}),M.reg(["linestart","spacing","indenting","quoted"])("newline","linestart",function(n){l.closebraces()})("eos","eos",function(n){l.closebraces(),d()}),M.reg(["symboling"])("eos","eos",function(n){s.end(),l.closebraces(),d()})
var T=[]
return e.on("list",function(n){T.push(n)}),{parse:t,show_state_map:n}}()
t.exports=k
var x=function(n){return q(a(4*n).map(function(n){return" "}).join(""))},L="",q=function(n){L+=n},E=function(n,t){t=t||0,o("symbol"===n.type,function(){q(n.symbol+" ")},"string"===n.type,function(){q('"'+n.string+'"')},"list"===n.type,function(){q("\n"),x(t),q("("),n.children.map(function(n){E(n,t+1)}),q(")")},function(){throw new Error("unknown node type")})()}
